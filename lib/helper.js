  ...
  */
  ...
  convertToPmBody: function(ramlBody, types, options) {
    ...
    if (ramlBody.hasOwnProperty(URLENCODED)) {
      ...
      bodyData = this.getBodySchema(ramlBody[URLENCODED], types, options.requestResolution);
      ...
      rDataMode = 'formdata';
      bodyData = this.getBodySchema(ramlBody[FORM_DATA], types, options.requestResolution);
      ...
      bodyData = this.getBodySchema(ramlBody[bodyType], types, options.requestResolution);
  ...
  convertResponse: function(responses, originalRequest, types, options) {
    ...
    _.forEach(responses, (res) => {
      ...
      _.forEach(res.headers, (header) => {
        responseHeaders.push(helper.convertHeader(header, types, options.exampleResolution));
      });

      responseBodyType = Object.keys(_.get(res, 'body', {}))[0];

      if (responseBodyType) {
        responseBody = this.getBodySchema(res.body[responseBodyType], types, options.exampleResolution);
  ...
  */
  convertRequestToItem: function(method, url, globalParameters, options, pathVariables) {
    ...
    if (method.queryParameters) {
      url = url.concat(helper.constructQueryStringFromQueryParams(
        method.queryParameters, globalParameters.types, options.requestResolution
      ));
    }
    if (method.queryString) {
      url = url.concat(helper.constructQueryString(
        method.queryString, globalParameters.types, options.requestResolution
      ));
    }
    ...
    _.forEach(method.headers, (header) => {
      request.headers.add(helper.convertHeader(header, globalParameters.types, options.requestResolution));
    });

    // pass request.toJSON() as extra fields are added in response for original request object
    _.forEach(helper.convertResponse(method.responses, request.toJSON(), globalParameters.types, options), (res) => {
