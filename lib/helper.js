...
helper = {
  ...
  */
  convertBody: function(ramlBody, types) {
    ...
    else if (ramlBody['text/xml']) {
      body = !_.isEmpty(body) ? body : 'text/xml not supported';
    }
    else {
  ...
  constructQueryStringFromQueryParams: function(queryParameters, types) {
    ...
    _.forEach(queryParameters, (param, key) => {
      if (param.default || param.example) {
        ...
        // resolve for object type of value
        fakedSchema = safeSchemaFaker(param, types);
        if (_.isObject(fakedSchema)) {
          _.forEach(fakedSchema, (value, key) => {
            SDKUrl.addQueryParams(new SDK.QueryParam({
              key: key,
              value: value
            }));
  ...
  */
  convertRequestToItem: function(method, url, globalParameters, options, pathVariables) {
    ...
    convertedResponses = method.responses && helper.convertResponse(method.responses, globalParameters.types);
    _.forEach(convertedResponses, (res) => {
